...
if [[ ${SCRIPT_NAME} = ${CASE_ARG} ]] ; then
    CONFIG_FILE="CDACreateTablesCaseICDC.yaml"
    SCRIPT_FILE="create_tables_case_metadata_icdc.py"
...
#    SCRIPT_FILE="create_tables_cohort_icdc.py"
    ...
    CONFIG_FILE="CDACreateTablesPerSampleFileICDC.yaml"
    SCRIPT_FILE="create_tables_per_sample_file_icdc.py"
...
cd ..
python3.11 ./BQ_Table_Building/CDA/ICDC/${SCRIPT_FILE} ~/config/${SHARED_CONFIG_FILE} ~/config/${CONFIG_FILE}
deactivate