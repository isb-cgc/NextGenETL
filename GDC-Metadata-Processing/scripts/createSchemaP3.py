...
#
# This version of the script has been modified to work with Python 3, and
# can also be called as a function within python
#
...
def main(args):
    ...
    build_schema(args[1], nSkip)

# --------------------------------------------------------------

def build_schema(inFilename, nSkip):

    ## scratch file ...
    dmpFh = open("subsample.tsv", 'w')
    ...
    for ii in range(len(fieldNames)):
        ...
        if (dataTypes[ii] == "NA"):
            dataTypes[ii] = "string"
        retval.append((fieldNames[ii], dataTypes[ii]))
        outStr = '    {"name": "' + fieldNames[ii] + '", "type": "' + dataTypes[
            ii] + '", "mode": "nullable", "description": "<add description here>"}'
