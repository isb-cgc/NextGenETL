...
Building a TEST File Metadata Table for Reverse Engineering
This routine uses the format for tables <= release 13.
'''
...
'''
----------------------------------------------------------------------------------------------
BAM and VCF extraction (44402 BAMs, 44505 simple somatic and 44505 annotated somatic VCFs in the target TCGA table)
'''
...
SQL for above:
'''
def extract_file_data_sql_archived_aligned(release_table, program_name):
    return '''
        SELECT
...
'''
----------------------------------------------------------------------------------------------
Slide extraction (30036 slides in the target TCGA table)
'''
...
'''
----------------------------------------------------------------------------------------------
Clinical extraction (11158 CLIN, 11309 BIO files in the target TCGA HG19 LEGACY table ONLY):
'''
...
----------------------------------------------------------------------------------------------
SQL for above:
Use this to figure out that the target table was always using the second of the two sample IDs
'''
def aliquot_barcodes_sql_archive_too(release_table, aliquot_2_case_table, program_name):
    #
    # Use this to figure out that the target table was always using the second of the two sample IDs
    #
    return '''
        ...
        SELECT
            ...
            a1.sample_gdc_id,
            #a1.sample_barcode,
            REGEXP_EXTRACT(a1.sample_barcode, r"^TCGA-[0-9]+-[0-9]+-([a-zA-Z0-9]+)$") as sample_barcode,
            c.sample_gdc_id as sample_gdc_id_too,
            REGEXP_EXTRACT(c.sample_barcode, r"^TCGA-[0-9]+-[0-9]+-([a-zA-Z0-9]+)$") as sample_barcode_too,
            #c.sample_barcode as sample_barcode_too,
            ...
            a1.access,
            a1.acl
        FROM a1 JOIN `{1}` AS c ON a1.aliquot_id_2 = c.aliquot_gdc_id
        '''.format(release_table, aliquot_2_case_table, program_name)
