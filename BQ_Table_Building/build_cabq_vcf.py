...
def load_to_production_env(publish_project,
                           ...
                           labels_and_desc):
    '''
        @paramaters publish_project, publish_dataset_id, 
                    publish_table_id, schema, scratch_table_id, labels_and_desc
                    

        Final version of the table will uploaded to the publish project with the 
        labels, description, and schema description. 
...
def load_to_staging_env(dataset_id,
    ...
    '''
        @paramaters dataset_id, table_id, bucket_path, schema_path

        An initial upload of the table will be pushed into BigQuery from Storage 
        using Google's command line funtions.  
...
def push_file_to_bucket(final_merged_csv,
    ...
    subprocess.call(f"gsutil -o GSUtil:parallel_composit_upload_threshold=150M cp  {final_merged_csv} {bucket_path}", 
                    shell = True)
...
def generate_a_simple_schema(file_path_to_json,
    ...
    '''
        @paramaters file_path_to_json, dataframe_information_file

        Using the dataframe information derived through Pandas the schema 
        will be updated and sent to a json file. 
...
def simple_schema_builder(program_name,
    ...
    '''
        @paramaters program_name, final_merged_csv, dataframe_information_file, file_path 

        Builds simple schema using Pandas to detect information and manualy updates the schema. 
    ...
    elif program_name.lower() == 'hcmi':
        file_path_to_json = f'{home}/NextGenETL/intermediateFiles/varepop_simple_build_schema.json'
    elif program_name.lower() == 'organoid':
        file_path_to_json = f'{home}/NextGenETL/intermediateFiles/organoid_simple_build_schema.json'
    elif program_name.lower() == 'cptac':
        file_path_to_json = f'{home}/NextGenETL/intermediateFiles/cptac_simple_build_schema.json'
    elif program_name.lower() == 'beataml1.0':
        file_path_to_json = f'{home}/NextGenETL/intermediateFiles/beataml1.0_simple_build_schema.json'
    elif program_name.lower() == 'mmrf':
        file_path_to_json = f'{home}/NextGenETL/intermediateFiles/mmrf_simple_build_schema.json'
    else:
...
def generate_dataframe(column_headers, 
    ...
    for column in vcf_df.columns:
        if '_aliquot' in column or '-ALIQUOT' in column or 'TARGET' in column or 'ALIQUOT' in column:
        ...
        elif re.search("[A-Z].\d+.\d.+.\d.",column):
           vcf_df.rename(columns={column:'TUMOR'},inplace=True)
        else:
            vcf_df.rename(columns={column:column})
    
    if add_normal_col == True:
...
def query_for_table(filedata_active,
    ...
    query_job = (f'''
        ...
        WHERE 
            ...
            #AND f1.analysis_workflow_type in ("GATK4 MuTect2", "SomaticSniper")
       #LIMIT 2
       ''')
